...
- 2 equally-probable outcomes: $p = \frac{1}{n} = \frac{1/2}$, and uncertainty is $\frac{1}{\frac{1/2}) = 2$. 
- 1000 equally-probable outcomes. $p = \frac{1}{n} = \frac{1/1000}$, and uncertainty is $\frac{1}{\frac{1/2}) = 1000$. 
- 75% chance of rain: uncertainty $\frac{1}{.75) = 1.33$. 
- 99% chance of rain: uncertainty $\frac{1}{.99) ≈ 1.01$. 

So the uncertainty associated with a 99% chance of rain is approximately equal to the uncertainty associated with only $n = 1$ possible outcome. The uncertainty associated with 50% chance of rain is equal to that associated with $n = 2$ equally-probable outcomes. And the uncertainty associated with a 75% chance of rain is equal to that associated with $n = 1.33$ possible outcomes.

Now we are actually very close to the actual definition of Shannon entropy. We just have two more steps:

1. convert to a log scale.
...
First, because when there is only 1 possible outcome, it seems intuitive that uncertainty should be zero. And sure enough, $log(1) = 0$! 
...
To get the total number of possible, we need to count the number of culprit-weapon **combinations** (Prof. Plub with the lead pipe, etc). This means we need to multiply. 
...
Suppose I know who the murderer is. But you don't -- for you there are still two possibilities. How many bits of information do I need to provide to you to tell you who did it? Just one. I might send you a "1" for Professor Plumb and "0" for Colonel Mustard. I need to give you 1 bit of information to resolve your 1 bit of uncertainty about the murderer.

How many bits do I need to tell you who the murder weapon is? We said above there are four possibilities, and a 2 bit number can encode 4 possibilities. So again, I need to provide 2 bits of information to resolve your 2 bits of uncertainty about the weapon.

"Uncertainty" and "information" are two sides of the same coin. 

So every time you receive a byte of information, you can look at it as resolving 1 bit of uncertainty. For example, suppose I am sending you a byte of information, one bit at a time. Initially there are $2^8 = 256$ possible values for that byte. So your uncertainty is $log(256) = 8$ bits. When you find out the value of the first bit, you have cut the number of possible outcomes in half to $2^7 = 128$. Which means uncertainty is now $log(128) = 7$ bits. So each bit of information resolves 1 bit of uncertainty. 


## Uncertainty for Unequal Probabilities

If there are more than two possible outcomes, using the surprisal of the most probable outcome is actually a pretty good *approximation* of uncertainty. But it's not quite right. Say the most probable outcome is 60%. If there is only 1 other possible outcome, then there is much less uncertainty than if there were 10 more possible outcomes.
...
Which generalizes to Formula 5 above.
...
Shannon Entropy generalizes this measure when outcomes are not equally probable. When there is uncertainty about an outcome, Shannon entropy gives us the expected number of bits required to communicate the actual outcome using efficient coding.

Information can thus be seen as the resolution of uncertainty. Entropy is the number of bits of information required to resolve all uncertainty.
